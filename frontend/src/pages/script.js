...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const appState = {
        ...
        circles: [
            {
                ...
                description: "Изучение классической и современной литературы, обсуждение произведений",
        ...
        grades: [
            { id: 1, circleId: 1, studentId: 1, value: 5, comment: "Отличная работа над проектом", date: "2023-05-15" },
            { id: 2, circleId: 2, studentId: 1, value: 4.5, comment: "Хорошо, но есть над чем работать", date: "2023-05-10" },
            { id: 3, circleId: 1, studentId: 2, value: 4, comment: "Старайтесь больше практиковать разговорную речь", date: "2023-05-05" }
        ],
        materials: [
            { id: 1, circleId: 1, title: "Список литературы", description: "Рекомендуемые произведения для чтения", file: "literature-list.pdf" },
            { id: 2, circleId: 2, title: "Основы программирования", description: "Введение в Python", file: "python-basics.pdf" }
        ],
        messages: [
            { id: 1, from: "admin", to: 1, subject: "Добро пожаловать", text: "Добро пожаловать на платформу Логос Гим!", date: "2023-05-01" },
            { id: 2, from: 1, to: "admin", subject: "Вопрос по кружку", text: "Когда начинаются занятия по робототехнике?", date: "2023-05-02" }
        ],
        events: [
            { id: 1, circleId: 1, title: "Литературная викторина", date: "2023-05-25", time: "16:00-18:00", location: "Актовый зал" },
            { id: 2, circleId: 2, title: "Турнир по робототехнике", date: "2023-05-30", time: "12:00-15:00", location: "Лаборатория 203" }
        ],
        portfolio: [
            { id: 1, studentId: 1, title: "Мой первый проект", description: "Анализ произведения 'Преступление и наказание'", file: "project1.pdf" }
        ]
    };
    ...
    function setupEventListeners() {
        ...
        // Профиль
        DOM.userAvatarBtn.addEventListener('click', () => showModal(DOM.profileModal));
        DOM.editAvatarBtn.addEventListener('click', () => showModal(DOM.profileModal));
        ...
        DOM.regUserType.addEventListener('change', function() {
            DOM.adminCodeGroup.style.display = this.value === 'admin' ? 'block' : 'none';
        });
    ...
    function loadAdminDashboard(section) {
        ...
        switch (section) {
            ...
            case 'grades':
                content = `
                    ...
                    const student = appState.students.find(s => s.id === grade.studentId);
                    const circle = appState.circles.find(c => c.id === grade.circleId);
                    return `
                                                <tr>
                                                    <td>${student ? student.name : 'Неизвестно'}</td>
                                                    <td>${circle ? circle.title : 'Неизвестно'}</td>
                                                    <td>${grade.value}</td>
                                                    <td>${grade.date}</td>
                                                    <td class="actions-cell">
                                                        <div class="action-btn edit-btn" data-id="${grade.id}"><i class="fas fa-edit"></i></div>
                                                        <div class="action-btn delete-btn" data-id="${grade.id}"><i class="fas fa-trash"></i></div>
                                                    </td>
                                                </tr>
                                            `;
                }).join('')}
            ...
            case 'schedule':
                content = `
                    ...
                    const circle = appState.circles.find(c => c.id === event.circleId);
                    return `
                                                <tr>
                                                    <td>${event.title}</td>
                                                    <td>${circle ? circle.title : 'Неизвестно'}</td>
                                                    ...
                                                    <td class="actions-cell">
                                                        <div class="action-btn edit-btn" data-id="${event.id}"><i class="fas fa-edit"></i></div>
                                                        <div class="action-btn delete-btn" data-id="${event.id}"><i class="fas fa-trash"></i></div>
                                                    </td>
                                                </tr>
                                            `;
                }).join('')}
            ...
            case 'materials':
                content = `
                    ...
                    const circle = appState.circles.find(c => c.id === material.circleId);
                    return `
                                        <div class="material-card">
                                            ...
                                            <p><small>Кружок: ${circle ? circle.title : 'Неизвестно'}</small></p>
                                            <div class="material-actions">
                                                <button class="btn transparent-btn download-btn" data-id="${material.id}"><i class="fas fa-download"></i></button>
                                                <button class="btn transparent-btn edit-btn" data-id="${material.id}"><i class="fas fa-edit"></i></button>
                                                <button class="btn transparent-btn delete-btn" data-id="${material.id}"><i class="fas fa-trash"></i></button>
                                            </div>
                                        </div>
                                    `;
                }).join('')}
            ...
            case 'messages':
                content = `
                            ...
                            <div class="messages-container">
                                ${appState.messages.filter(msg => msg.from === 'admin' || msg.to === 'admin').map(message => {
                    const isOutgoing = message.from === 'admin';
                    const student = isOutgoing ? appState.students.find(s => s.id === message.to) : appState.students.find(s => s.id === message.from);
                    return `
                                        <div class="message">
                                            <div class="message-header">
                                                <span class="message-sender">${isOutgoing ? 'Вы → ' + (student ? student.name : 'Ученик') : (student ? student.name : 'Ученик') + ' → Вы'}</span>
                ...
                content = `
                    ...
                    return `
                                                <tr>
                                                    ...
                                                    <td class="actions-cell">
                                                        <div class="action-btn view-btn" data-id="${item.id}"><i class="fas fa-eye"></i></div>
                                                        <div class="action-btn delete-btn" data-id="${item.id}"><i class="fas fa-trash"></i></div>
                                                    </td>
                                                </tr>
                                            `;
                }).join('')}
        ...
        if (section === 'grades') {
            document.getElementById('addGradeBtn').addEventListener('click', () => showAddGradeModal());
            document.querySelectorAll('.edit-btn').forEach(btn => {
                btn.addEventListener('click', function() {
        ...
        if (section === 'materials') {
            document.getElementById('addMaterialBtn').addEventListener('click', () => showAddMaterialModal());
            document.querySelectorAll('.edit-btn').forEach(btn => {
                btn.addEventListener('click', function() {
    ...
    function loadStudentDashboard(section) {
        ...
        switch (section) {
                ...
                content = `
                            ...
                            <div class="dashboard-stats">
                                <div class="stat-card">
                                    ...
                                    <div class="stat-number">${appState.circles.filter(c => c.participants > 0).length}</div>
                                ...
                                <div class="stat-card">
                                    ...
                                    <div class="stat-number">${appState.messages.filter(m => m.to === appState.currentUser.id || m.from === appState.currentUser.id).length}</div>
            ...
            case 'circles':
                content = `
                            ...
                            <div class="circles-grid">
                                ${appState.circles.filter(c => c.participants > 0).map(circle => `
            ...
            case 'grades':
                const studentGrades = appState.grades.filter(g => g.studentId === appState.currentUser.id);
                content = `
                            <div class="section-header">
                                <h2>Мои оценки</h2>
                            </div>
                    ...
                    return `
                                        <div class="grade-card">
                                            <div class="grade-header">
                                                <h4>${circle ? circle.title : 'Неизвестный кружок'}</h4>
                    ...
                    return `
                                        <div class="event-card">
                                            ...
                                            <div class="event-meta">
                                                <span><i class="fas fa-book-open"></i> ${circle ? circle.title : 'Неизвестный кружок'}</span>
            ...
            case 'messages':
                const studentMessages = appState.messages.filter(m => m.to === appState.currentUser.id || m.from === appState.currentUser.id);
                content = `
                            <div class="section-header">
                                <h2>Мои сообщения</h2>
                                <button class="btn gold-btn" id="newMessageBtn"><i class="fas fa-plus"></i> Новое сообщение</button>
                    ...
                    const isOutgoing = message.from === appState.currentUser.id;
                    const student = isOutgoing ?
                        (message.to === 'admin' ? {name: 'Администратор'} : appState.students.find(s => s.id === message.to)) :
                        (message.from === 'admin' ? {name: 'Администратор'} : appState.students.find(s => s.id === message.from));
                    return `
                                        <div class="message">
                                            <div class="message-header">
                                                <span class="message-sender">${isOutgoing ? 'Вы → ' + (student ? student.name : 'Получатель') : (student ? student.name : 'Отправитель') + ' → Вы'}</span>
                                                <span class="message-date">${message.date}</span>
                                            </div>
                                            <h4 class="message-subject">${message.subject}</h4>
                                            <p class="message-text">${message.text}</p>
            ...
            case 'portfolio':
                const studentPortfolio = appState.portfolio.filter(p => p.studentId === appState.currentUser.id);
                content = `
                            <div class="section-header">
                                <h2>Мое портфолио</h2>
    ...
    function updateSidebarMenu() {
        ...
        if (appState.currentUser.role === 'admin') {
            ...
            menuItems = [
                ...
                { icon: 'fas fa-calendar-alt', text: 'Мое расписание', page: 'schedule' },
        ...
        DOM.sidebarMenu.innerHTML = menuItems.map(item => `
                    <li>
                        <a href="#" data-page="${item.page}" class="${item.page === 'profile' ? 'active' : ''}">
        ...
        DOM.sidebarMenu.querySelectorAll('a').forEach(link => {
            link.addEventListener('click', function(e) {
                ...
                DOM.sidebarMenu.querySelectorAll('a').forEach(a => a.classList.remove('active'));
    ...
    function loadCircles() {
        DOM.circlesGrid.innerHTML = appState.circles.map(circle => `
                    <div class="circle-card">
                        <div class="circle-image">
                            <img src="${circle.image}" alt="${circle.title}">
                            <span class="circle-category">${circle.category}</span>
                        ...
                        <div class="circle-info">
                            ...
                            <div class="circle-actions">
                                <button class="btn transparent-btn view-circle-btn" data-circle-id="${circle.id}">Подробнее</button>
                                <button class="btn gold-btn join-circle-btn" data-circle-id="${circle.id}">Записаться</button>
                            </div>
                        </div>
                    </div>
                `).join('');
    ...
    function viewCircle(circleId) {
        ...
        DOM.circleModalContent.innerHTML = `
                    ...
                    <div class="circle-info">
                        <p><strong>Категория:</strong> ${circle.category}</p>
                        <p><strong>Расписание:</strong> ${circle.schedule}</p>
                    ...
                    <div class="form-actions">
                        ...
                        ${appState.isLoggedIn ?
            `<button class="btn gold-btn join-circle-btn" data-circle-id="${circle.id}">Записаться</button>` :
            `<button class="btn gold-btn" id="loginFromCircleBtn">Войти для записи</button>`
        }
                    </div>
                `;
        ...
        if (appState.isLoggedIn) {
            document.querySelector('.join-circle-btn').addEventListener('click', function() {
                joinCircle(circleId);
                hideModal(DOM.circleModal);
            });
        } else {
            document.getElementById('loginFromCircleBtn').addEventListener('click', function() {
                hideModal(DOM.circleModal);
                showModal(DOM.loginModal);
            });
        }
    ...
    function joinCircle(circleId) {
        ...
        if (circle) {
            circle.participants++;
            showToast('Вы успешно записались в кружок', 'success');
            loadCircles();
    ...
    function showEditCircleModal(circleId) {
        ...
        DOM.circleModalContent.innerHTML = `
                    ...
                    <form id="editCircleForm">
                        <div class="form-group">
                            ...
                            <input type="text" id="editCircleTitle" class="form-control" value="${circle.title}" required>
                        </div>
                        <div class="form-group">
                            <label for="editCircleDescription">Описание</label>
                            <textarea id="editCircleDescription" class="form-control" rows="3" required>${circle.description}</textarea>
                        </div>
                        <div class="form-row">
                            <div class="form-group">
                                <label for="editCircleCategory">Категория</label>
                                <select id="editCircleCategory" class="form-control" required>
                                    <option value="Гуманитарные" ${circle.category === 'Гуманитарные' ? 'selected' : ''}>Гуманитарные</option>
                                    <option value="Технические" ${circle.category === 'Технические' ? 'selected' : ''}>Технические</option>
                                    <option value="Творческие" ${circle.category === 'Творческие' ? 'selected' : ''}>Творческие</option>
                                    <option value="Спортивные" ${circle.category === 'Спортивные' ? 'selected' : ''}>Спортивные</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="editCircleSchedule">Расписание</label>
                                <input type="text" id="editCircleSchedule" class="form-control" value="${circle.schedule}" required>
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="editCircleImage">Изображение (URL)</label>
                            <input type="text" id="editCircleImage" class="form-control" value="${circle.image}" required>
                        </div>
                        <div class="form-actions">
                            <button type="button" class="btn transparent-btn close-modal-btn">Отмена</button>
                            <button type="submit" class="btn gold-btn">Сохранить</button>
        ...
        document.getElementById('editCircleForm').addEventListener('submit', function(e) {
            e.preventDefault();

            circle.title = document.getElementById('editCircleTitle').value;
            circle.description = document.getElementById('editCircleDescription').value;
    ...
    function showAddGradeModal() {
        ...
        circleSelect.innerHTML = appState.circles.map(c =>
    ...
    function showEditGradeModal(gradeId) {
        ...
        const circleSelect = document.getElementById('gradeCircle');
        circleSelect.innerHTML = appState.circles.map(c =>
            `<option value="${c.id}" ${c.id === grade.circleId ? 'selected' : ''}>${c.title}</option>`
        ).join('');
    ...
    function showEditEventModal(eventId) {
        ...
        circleSelect.className = 'form-control';
        circleSelect.innerHTML = appState.circles.map(c =>
            `<option value="${c.id}" ${c.id === event.circleId ? 'selected' : ''}>${c.title}</option>`
        ).join('');
    ...
    function showAddMaterialModal() {
        ...
        document.getElementById('materialForm').addEventListener('submit', function(e) {
            ...
            const fileName = fileInput.files.length > 0 ? fileInput.files[0].name : 'material.pdf';
    ...
    function showEditMaterialModal(materialId) {
        ...
        circleSelect.className = 'form-control';
        circleSelect.innerHTML = appState.circles.map(c =>
            `<option value="${c.id}" ${c.id === material.circleId ? 'selected' : ''}>${c.title}</option>`
        ).join('');
        ...
        document.getElementById('materialForm').addEventListener('submit', function(e) {
            ...
            const fileInput = document.getElementById('materialFile');
            const fileName = fileInput.files.length > 0 ? fileInput.files[0].name : material.file;

            material.title = document.getElementById('materialTitle').value;
            material.description = document.getElementById('materialDescription').value;
            material.circleId = parseInt(document.getElementById('materialCircle').value);
    ...
    function showNewMessageModal() {
        ...
        if (appState.currentUser.role === 'admin') {
            ...
            recipientSelect.innerHTML = appState.students.map(s =>
                `<option value="${s.id}">${s.name}</option>`
            ).join('');
        } else {
            // Студент может писать только администратору
            recipientSelect.innerHTML = '<option value="admin">Администратор</option>';
        }
        ...
        document.getElementById('recipientSelectContainer').appendChild(recipientSelect);
        showModal(DOM.circleModal);
        ...
        document.getElementById('messageForm').addEventListener('submit', function(e) {
            ...
            const newMessage = {
                ...
                from: appState.currentUser.role === 'admin' ? 'admin' : appState.currentUser.id,
    ...
    function showAddPortfolioModal() {
        ...
        document.getElementById('portfolioForm').addEventListener('submit', function(e) {
            ...
            const fileName = fileInput.files.length > 0 ? fileInput.files[0].name : 'work.pdf';
    ...
    function showEditPortfolioModal(portfolioId) {
        ...
        document.getElementById('portfolioForm').addEventListener('submit', function(e) {
            ...
            const fileInput = document.getElementById('portfolioFile');
            const fileName = fileInput.files.length > 0 ? fileInput.files[0].name : portfolioItem.file;

            portfolioItem.title = document.getElementById('portfolioTitle').value;
            portfolioItem.description = document.getElementById('portfolioDescription').value;
    ...
    function viewPortfolioItem(portfolioId) {
        ...
        document.querySelector('.download-portfolio-btn').addEventListener('click', function() {
    ...
    function deletePortfolioItem(portfolioId) {
        if (confirm('Вы уверены, что хотите удалить эту работу из портфолио?')) {
            appState.portfolio = appState.portfolio.filter(p => p.id !== portfolioId);
    ...
    function updateUI() {
        if (appState.isLoggedIn) {
            ...
            if (appState.currentUser) {
                ...
                document.getElementById('userRole').textContent = appState.currentUser.role === 'admin' ? 'Администратор' : 'Участник';
